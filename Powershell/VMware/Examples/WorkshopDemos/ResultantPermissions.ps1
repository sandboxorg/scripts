...
# Reporting on roles and permissions.
Get-ResultantPrivileges -principal "VMWORLD\cshanklin" -viobject (Get-VM OpenFiler)
Get-VM Openfiler | Get-VIPermission
Get-ResultantPrivileges -principal "VMWORLD\asharif" -viobject (Get-VM OpenFiler)
Get-VM Openfiler | New-VIPermission -Principal "VMWORLD\asharif" -Role ReadOnly
Get-ResultantPrivileges -principal "VMWORLD\asharif" -viobject (Get-VM OpenFiler)
get-vm Openfiler | Get-VIPermission | select -First 1 | Remove-VIPermission
